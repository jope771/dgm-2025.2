# Exploração do Manifold de Emoções a partir de Expressões Faciais
# Emotions Manifold Exploration from Facial Expressions
...
3.  
1.  **Encoder Network:** Input Image $x$ $\rightarrow$ Latent Vector $z$ ($\mu, \sigma$).
2.  **Manifold Mapping (Linear Probe):**
    * Dataset of pairs $(z_i, \text{Valence}_i)$ and $(z_i, \text{Arousal}_i)$.
    * Linear Regression to find direction vectors $d_{val}$ and $d_{aro}$.
3.  **Orthogonalization:** Gram-Schmidt process applied to $d_{aro}$ relative to $d_{val}$ to ensure independent control.
4.  **Exploration Engine:**
    * Input: Seed Face $z_{seed}$.
    * Operation: $z_{new} = z_{seed} + (\alpha \cdot d_{val}) + (\beta \cdot d_{aro})$.
    * Decoder Network: $z_{new} \rightarrow$ Generated Image.

## Experiments, Results, and Discussion of Results

### 1. Manifold Linearity Analysis
To verify if the latent space actually encodes emotions linearly as posited by Russell's model, we measured the $R^2$ score of our linear probes.
...
* **Discussion:** The center of the grid represents the "Neutral" face. As we move to the top-left (High Arousal, Low Valence), the face naturally morphs into a distressed/fearful expression. As we move to the bottom-right (Low Arousal, High Valence), the face relaxes into a calm satisfaction. This visual proof confirms the model learned the **topology** of human affect without explicit rule-based programming.

## Conclusion

This project successfully demonstrated that the **Russell Circumplex Model** is not just a psychological construct but a realizable geometric structure within the latent space of deep generative models. By aligning the mathematical axes of a VAE with the psychological axes of Valence and Arousal, we created a tool for **Emotion Manifold Exploration**.

The resulting system allows for the synthesis of nuanced emotional states that lie between standard categories, offering a more granular approach to facial expression generation. Future work will investigate non-linear mappings (using normalizing flows) to capture the curvature of the emotional manifold more accurately.

## Bibliographic References
1.  **Russell, J. A. (1980).** "A circumplex model of affect". *Journal of Personality and Social Psychology*. (The foundational theory for our manifold alignment).
2.  **Mollahosseini, A., et al. (2017).** "AffectNet: A Database for Facial Expression, Valence, and Arousal Computing in the Wild".
3.  **Kingma, D. P., & Welling, M. (2013).** "Auto-Encoding Variational Bayes". (The backbone of our generative architecture).
4.  **Radford, A., et al. (2015).** "Unsupervised Representation Learning with Deep Convolutional Generative Adversarial Networks". (Reference for latent vector arithmetic).
